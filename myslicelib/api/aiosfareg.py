...
from myslicelib.api.aiosfa import Sfa as SfaApi
...
class AiosfaReg(SfaApi):
    ...
    def __init__(self, endpoint, authentication):
        ...
        if os.path.isfile(authentication.certificate):
            ...
            self.certificate = authentication.certificate
        ...
        #     for cred in authentication.credentials:
        #         if 'id' not in cred:
        #             cred['id'] = hrn_to_urn(cred['hrn'], cred['type'])
        #         if 'hrn' not in cred:
        #             cred['hrn'], cred['type'] = urn_to_hrn(cred['id'])
        #         if ('xml' in cred and
        #             isinstance(cred['xml'], str) and
        #             os.path.isfile(cred['xml'])):

        #             with open(cred['xml'], "r") as myfile:
        #                 cred['xml'] = myfile.read()
        #         # Check if the credential is expired
        #         el = xml.etree.ElementTree.fromstring(cred['xml'])
    ...
    async def _get_user_credential(self):
        # print('GetSelfCredential from Registry')
        self.user_credential = await self._proxy.GetSelfCredential(
                                    self.certificate,
                                    self.authentication.hrn,
                                    'user')
        return self.user_credential

    async def _get_entities(self, entity):
        self.user_credential = await self._get_user_credential()

        version = await self._proxy.GetVersion()
        platform_urn = version['urn']

        records = await self._proxy.List(platform_urn, self.user_credential,
                                         {'recursive': True})
        if entity == 'authority':
    ...
    async def _get_entity(self, hrn, entity):
        self.user_credential = await self._get_user_credential()

        if entity in ['authority', 'project']:
            records = await self._proxy.List(hrn, self.user_credential,
                                             {'recursive': True})
            list_data = getattr(self, "_filter_" + entity)(records)
        else:
            list_data = await self._proxy.Resolve(hrn, self.user_credential,
                                                  {})
    ...
    async def _parse_slice(self, list_data):
        slices = []
        for sli in list_data:
            ...
            for usr in sli.get('reg-researchers', []):
                ...
                usr_record = await self._get_entity(usr, "user")
                geni_users.append({
                                    'urn': usr_urn,
    ...
    async def get(self, entity, urn=None, raw=False):
        result = []
        try:
            if urn is None:
                result = await self._get_entities(entity)
            else:
                hrn, type = urn_to_hrn(urn)
                if type not in self._types:
                    raise MysNotUrnFormatError('not valid urn')
                result = await self._get_entity(hrn, entity)
            if not raw:
                if entity == 'slice':
                    # A spceical case for slice
                    result = await self._parse_slice(result)
                else:
    ...
    async def delete(self, entity, urn):
        result = []
        if entity == 'project':
            entity = 'authority'
        try:
